...
(() => {
  ...
  const makeFallbackPoster = (label = "Berry Vibes Recipe") => {
    const safe = String(label || "Berry Vibes Recipe").replace(/[<>&"']/g, "").slice(0, 54);
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="900" height="1050" viewBox="0 0 900 1050">
        ...
        <rect width="900" height="1050" fill="url(#g)"/>
        <rect x="28" y="28" width="844" height="994" rx="42" fill="none" stroke="#ed5f89" stroke-width="5"/>
        <circle cx="720" cy="180" r="90" fill="#ffd2df"/>
        <circle cx="720" cy="180" r="48" fill="#fff9f4"/>
        <text x="58" y="105" font-family="Arial,sans-serif" font-size="28" font-weight="700" fill="#ed5f89" letter-spacing="4">BERRY VIBES STUDIO</text>
        <text x="450" y="430" text-anchor="middle" font-family="Arial,sans-serif" font-size="82" font-weight="900" fill="#4a2630">🍓</text>
        <foreignObject x="90" y="520" width="720" height="230">
          <div xmlns="http://www.w3.org/1999/xhtml" style="font:900 54px/1.05 Arial,sans-serif;color:#4a2630;text-align:center;display:flex;align-items:center;justify-content:center;height:100%;">${safe}</div>
        </foreignObject>
        <text x="450" y="900" text-anchor="middle" font-family="Arial,sans-serif" font-size="24" font-weight="700" fill="#ed5f89" letter-spacing="3">LOCAL FALLBACK POSTER</text>
      </svg>`;
  ...
  const installImageFallbacks = () => {
    document.addEventListener("error", (event) => {
      ...
      const label = img.dataset.fallbackTitle || img.alt || "Berry Vibes Recipe";
      img.src = makeFallbackPoster(label);
  ...
  const createCategoryGate = (group, index) => {
    ...
    section.innerHTML = `
      <div>
        ...
        <p>Six recipe frames from ${escapeHtml(group)}. Scroll to let the contact sheet develop.</p>
      </div>
  ...
  const createRecipeFrame = (recipe, index) => {
    ...
    section.innerHTML = `
      <figure class="frame-image">
        <img src="${escapeHtml(recipe.poster)}" alt="Berry Vibes Studio poster for ${escapeHtml(recipe.name)}" data-fallback-title="${escapeHtml(recipe.name)}" loading="lazy" decoding="async">
      </figure>
  ...
  const renderIndex = () => {
    ...
    filtered.forEach((recipe) => {
      ...
      button.innerHTML = `
        <img src="${escapeHtml(recipe.poster)}" alt="" data-fallback-title="${escapeHtml(recipe.name)}" loading="lazy" decoding="async">
  ...
  const openRecipe = (index) => {
    ...
    dialogPoster.dataset.fallbackApplied = "false";
    dialogPoster.dataset.fallbackTitle = recipe.name;
    dialogPoster.src = recipe.poster;
    dialogPoster.alt = `Berry Vibes Studio poster for ${recipe.name}`;
  ...
  const initObserver = () => {
    ...
    window.setTimeout(() => {
      targets.filter((el) => el.getBoundingClientRect().top < window.innerHeight * 1.2)

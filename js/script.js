...
(() => {
  ...
  const createRecipeFrame = (recipe, index) => {
    ...
    section.innerHTML = `
      <figure class="frame-image">
        <img src="${escapeHtml(recipe.poster)}" alt="Berry Vibes Studio poster for ${escapeHtml(recipe.name)}" loading="lazy">
      </figure>
  ...
  const renderIndex = () => {
    ...
    filtered.forEach((recipe) => {
      ...
      button.innerHTML = `
        <img src="${escapeHtml(recipe.poster)}" alt="" loading="lazy">
  ...
  const openRecipe = (index) => {
    ...
    document.querySelector("#dialog-poster").alt = `Berry Vibes Studio poster for ${recipe.name}`;

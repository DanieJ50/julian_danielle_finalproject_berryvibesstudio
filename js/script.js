...
(() => {
  ...
  document.addEventListener("error", (event) => {
    ...
    image.alt = "BerryBelle fallback sketch image";
  }, true);
  ...
  const createRecipeFrame = (recipe, index) => {
    ...
    section.innerHTML = `
      <figure class="frame-image">
        <img src="${escapeHtml(recipe.poster)}" alt="Recipe sketch illustration representing ${escapeHtml(recipe.name)}" loading="lazy" decoding="async">
      </figure>
  ...
  const renderIndex = () => {
    ...
    filtered.forEach((recipe) => {
      ...
      button.innerHTML = `
        <img src="${escapeHtml(recipe.poster)}" alt="Recipe sketch illustration for ${escapeHtml(recipe.name)}" loading="lazy" decoding="async">
  ...
  const openRecipe = (index) => {
    ...
    document.querySelector("#dialog-poster").alt = `Recipe sketch illustration representing ${recipe.name}`;
